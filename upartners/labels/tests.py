...
class LabelTest(UPartnersTest):
    ...
    def test_create(self, mock_create_label, mock_get_labels):
        mock_get_labels.return_value = [
            TembaLabel.create(uuid='L-001', name="Not Ebola", parent=None, count=12),
            TembaLabel.create(uuid='L-002', name="EBOLA", parent=None, count=123)
        ]
        mock_create_label.return_value = TembaLabel.create(uuid='L-003', name="Chat", parent=None, count=0)
        ...
        self.assertEqual(ebola.words, 'ebola,fever')
        self.assertEqual(ebola.get_words(), ['ebola', 'fever'])
        ...
        self.assertEqual(Label.objects.get(pk=ebola.pk).uuid, 'L-002')

        # create local label with no match in RapidPro
        chat = Label.create(self.unicef, "Chat", "Chatting", ['chat'], [self.moh, self.who])
        self.assertEqual(Label.objects.get(pk=chat.pk).uuid, 'L-003')
...
class LabelCRUDLTest(UPartnersTest):
    def test_create(self):
        ...
        self.assertFormError(response, 'form', 'words', 'This field is required.')
        ...
        response = self.url_post('unicef', url, dict(name="Ebola", description="Msgs about ebola",
                                                     words="ebola,fever", partners=[self.moh.pk, self.who.pk]))
