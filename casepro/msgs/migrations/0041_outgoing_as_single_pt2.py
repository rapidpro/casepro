...
def split_outgoings(apps, schema_editor):
    ...
    for id_batch in chunks(outgoing_ids, 1000):
        outgoings = Outgoing.objects.filter(pk__in=id_batch).select_related('org', 'partner').prefetch_related('contacts')
            ...
            if len(contacts) > 0:
                ...
                if len(contacts) > 1:
                    for other_contact in contacts[1:]:
                        Outgoing.objects.create(org=outgoing.org,
                    ...
                    print("Split outgoing message #%d into %d single-recipient messages" % (outgoing.pk, len(contacts)))


class Migration(migrations.Migration):

    dependencies = [
        ('msgs', '0040_outgoing_as_single_pt1'),
    ]

    operations = [
        migrations.RunPython(split_outgoings)
    ]
